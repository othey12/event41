...
export async function GET(
  ...
  try {
    ...
    const [participantRows] = await db.execute(
      'SELECT COUNT(*) AS `total` FROM participants p JOIN tickets t ON p.ticket_id = t.id WHERE t.event_id = ? AND t.is_verified = TRUE',
      [eventId]
    );
    ...
    const [generatedRows] = await db.execute(
      'SELECT COUNT(*) AS `generated` FROM certificates c JOIN participants p ON c.participant_id = p.id JOIN tickets t ON p.ticket_id = t.id WHERE t.event_id = ? AND c.path IS NOT NULL',
      [eventId]
    );
    ...
    const [sentRows] = await db.execute(
      'SELECT COUNT(*) AS `sent` FROM certificates c JOIN participants p ON c.participant_id = p.id JOIN tickets t ON p.ticket_id = t.id WHERE t.event_id = ? AND c.sent = TRUE',
      [eventId]
    );
